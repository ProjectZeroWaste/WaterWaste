    ...
    for detection in detections:
        ...
        if score >= confidence_threshold:
            ...
            if 'classifications' in detection:
                ...
                clss = DetectorCFG.labelmap_num + int(detection['classifications'][0][0])
...
def render_iMerit_boxes(boxes, classes, image,label_map = DetectorCFG.label_id_name):
...
def render_megadb_bounding_boxes(boxes_info, image):
    ...
    for b in boxes_info:
        ...
        classes.append(DetectorCFG.label_name_id[b['category']])
