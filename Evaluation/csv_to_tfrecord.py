...
flags = tf.app.flags
flags.DEFINE_string('csv_input', '/mnt/omreast_users/phhale/csiro_trashnet/stormwater_vids/high_res_validation_set/_xml_to_csv_labels.csv', 'Path to the CSV input')
flags.DEFINE_string('image_dir', '/mnt/omreast_users/phhale/csiro_trashnet/stormwater_vids/high_res_validation_set/', 'Path to the image directory')
flags.DEFINE_string('output_path', '/mnt/omreast_users/phhale/csiro_trashnet/stormwater_vids/stormwater_vids.tfrecord', 'Path to output TFRecord')
FLAGS = flags.FLAGS

# TO-DO replace this with label map
def class_text_to_int(row_label):
    if row_label == 'H_beveragebottle':
        return 1
    elif row_label == 'D_lid':
        return 2
    elif row_label == 'S_cup':
        return 3
    elif row_label == 'P_foodcontainer':
        return 4
    elif row_label == 'P_beveragecontainer':
        return 5
    elif row_label == 'D_foodcontainer':
        return 6
    elif row_label == 'H_facemask':
        return 7
    elif row_label == 'M_aerosol':
        return 8
    elif row_label == 'H_otherbottle':
        return 9
    elif row_label == 'P_cup':
        return 10
    elif row_label == 'M_beveragecan':
        return 11
    else:
